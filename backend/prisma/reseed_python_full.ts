...
import { pythonSections, pythonFinalExam } from './content/python';
import { pythonTopicQuizzes } from './content/python_topic_quizzes';
...
/**
 * Full-replace reseed for the Python Programming & Scripting course
 * (issue #97).
 *
 * Replaces the machine-generated template content (near-duplicate quizzes from
 * seed.ts) with the deep, hand-written GfG-style curriculum in content/python.ts:
 ...
 * Python course are rebuilt. Idempotent: safe to run multiple times.
 */

const COURSE_ID = 'Python';

async function main() {
  console.log('--- FULL RE-SEED: Python Programming & Scripting (deep curriculum) ---');
  ...
  if (!course) {
    course = await prisma.course.create({
      data: {
        ...
        description:
          'Master Python from first principles to a shipped CLI tool — syntax, data structures, OOP, files & automation, pandas & Matplotlib, web data, and a tested, packaged project — with deep original lessons, working code, and real practice quizzes.',
        price: 699,
        isPublished: true,
      },
    });
  ...
  for (const section of pythonSections) {
    ...
    for (let i = 0; i < section.topics.length; i++) {
      ...
      const topicQuizzes = pythonTopicQuizzes[topic.title];
      if (!topicQuizzes) {
  ...
  for (const q of pythonFinalExam) {
  ...
  console.log(`\n--- PYTHON RE-SEED COMPLETE ---`);
  console.log(`Modules rebuilt:   ${pythonSections.length}`);
  ...
  console.log(`Final exam:        replaced ${oldExam.count} template questions → ${pythonFinalExam.length} distinct`);
}
