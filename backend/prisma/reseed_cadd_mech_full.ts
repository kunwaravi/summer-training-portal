...
/**
 * Full-replace reseed for the CADDED Software (Mechanical) course
 * (issue #94).
 *
 * Replaces the machine-generated template content (near-duplicate quizzes from
 * seed.ts) with the deep, hand-written GfG-style curriculum in content/cadd_mech.ts:
 *   - 20 modules (weeks) with original teaching topics covering AutoCAD,
 *     SolidWorks, CATIA, CNC G-code/M-code, and integrated mechanical drafting
 ...
 * CADDED_Mech course are rebuilt. Idempotent: safe to run multiple times.
 */

const COURSE_ID = 'CADDED_Mech';

async function main() {
  console.log('--- FULL RE-SEED: CADDED Software (Mechanical) (deep curriculum) ---');
  ...
  if (!course) {
    course = await prisma.course.create({
      data: {
        ...
        description:
          'Master AutoCAD, SolidWorks, CATIA, CNC Programming, and mechanical drafting systems.',
        price: 699,
        isPublished: true,
      },
    });
  ...
  for (const section of caddMechSections) {
    ...
    for (let i = 0; i < section.topics.length; i++) {
      ...
      const topicQuizzes = caddMechTopicQuizzes[topic.title];
      if (!topicQuizzes) {
