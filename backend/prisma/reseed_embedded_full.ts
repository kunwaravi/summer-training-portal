...
import { embeddedSections, embeddedFinalExam } from './content/embedded';
import { embeddedTopicQuizzes } from './content/embedded_topic_quizzes';
...
/**
 * Full-replace reseed for the Embedded Systems & Real-Time OS course
 * (issue #95).
 *
 * Replaces the machine-generated template content (near-duplicate quizzes from
 * seed.ts) with the deep, hand-written GfG-style curriculum in content/embedded.ts:
 ...
 * Embedded course are rebuilt. Idempotent: safe to run multiple times.
 */

const COURSE_ID = 'Embedded';

async function main() {
  console.log('--- FULL RE-SEED: Embedded Systems & Real-Time OS (deep curriculum) ---');
  ...
  if (!course) {
    course = await prisma.course.create({
      data: {
        ...
        description:
          'Architect microcontroller interfaces, serial communication buses, and RTOS kernels.',
        price: 699,
        isPublished: true,
      },
    });
  ...
  for (const section of embeddedSections) {
    ...
    for (let i = 0; i < section.topics.length; i++) {
      ...
      const topicQuizzes = embeddedTopicQuizzes[topic.title];
      if (!topicQuizzes) {
  ...
  for (const q of embeddedFinalExam) {
  ...
  console.log(`\n--- EMBEDDED RE-SEED COMPLETE ---`);
  console.log(`Modules rebuilt:   ${embeddedSections.length}`);
  ...
  console.log(`Final exam:        replaced ${oldExam.count} template questions → ${embeddedFinalExam.length} distinct`);
}
