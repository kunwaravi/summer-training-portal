...
import { cppSections, cppFinalExam } from './content/cpp';
import { cppTopicQuizzes } from './content/cpp_topic_quizzes';
...
/**
 * Full-replace reseed for the C++ & OOP for Embedded Systems course
 * (issue #91).
 *
 * Replaces the machine-generated template content (near-duplicate quizzes from
 * seed.ts) with the deep, hand-written GfG-style curriculum in content/cpp.ts:
...
async function main() {
  console.log('--- FULL RE-SEED: C++ & OOP for Embedded Systems (deep curriculum) ---');
  ...
  if (!course) {
    course = await prisma.course.create({
      data: {
        ...
        description:
          'Architect high-performance OOP software structures, customized template classes, memory-mapped placement new, and low-overhead collections — with 20 deep sections and hands-on quizzes.',
        price: 699,
        isPublished: true,
      },
    });
  ...
  for (const section of cppSections) {
    ...
    for (let i = 0; i < section.topics.length; i++) {
      ...
      const topicQuizzes = cppTopicQuizzes[topic.title];
      if (!topicQuizzes) {
  ...
  console.log(`\n--- C++ RE-SEED COMPLETE ---`);
  console.log(`Modules rebuilt:   ${cppSections.length}`);
  ...
  console.log(`Final exam:        replaced ${oldExam.count} template questions → ${cppFinalExam.length} distinct`);
}
