...
/**
 * Full-replace reseed for the Web Design & Frontend Development course
 * (issue #99).
 *
 * Replaces the machine-generated template content (near-duplicate quizzes from
 * seed.ts) with the deep, hand-written GfG-style curriculum in content/webdesign.ts:
 ...
 *   - A real 15-question final exam (replaces the near-identical templates)
 *
 ...
 * WebDesign course are rebuilt. Idempotent: safe to run multiple times.
 */

const COURSE_ID = 'WebDesign';

async function main() {
  console.log('--- FULL RE-SEED: Web Design & Frontend Development (deep curriculum) ---');
  ...
  if (!course) {
    course = await prisma.course.create({
      data: {
        ...
        description:
          'Build production-ready websites from scratch — semantic HTML, modern CSS (flexbox, grid, Tailwind), JavaScript interactivity, DOM, async fetch, and a deployed portfolio — with deep, original lessons, working code, and real practice quizzes.',
        price: 699,
        isPublished: true,
      },
    });
  ...
  for (const section of webdesignSections) {
    ...
    for (let i = 0; i < section.topics.length; i++) {
      ...
      const topicQuizzes = webdesignTopicQuizzes[topic.title];
      if (!topicQuizzes) {
