...
import { sqlSections, sqlFinalExam } from './content/sql';
import { sqlTopicQuizzes } from './content/sql_topic_quizzes';
...
/**
 * Full-replace reseed for the Database Management & SQL course
 * (issue #90).
 *
 * Replaces the machine-generated template content (near-duplicate quizzes from
 * seed.ts) with the deep, hand-written GfG-style curriculum in content/sql.ts:
 ...
 * SQL course are rebuilt. Idempotent: safe to run multiple times.
 */

const COURSE_ID = 'SQL';

async function main() {
  console.log('--- FULL RE-SEED: Database Management & SQL (deep curriculum) ---');
  ...
  if (!course) {
    course = await prisma.course.create({
      data: {
        ...
        description:
          'Learn relational databases, SQL queries, joins, indexes, transactions, and schema design in Hinglish — with 20 deep sections and hands-on quizzes.',
        price: 699,
        isPublished: true,
      },
    });
  ...
  for (const section of sqlSections) {
    ...
    for (let i = 0; i < section.topics.length; i++) {
      ...
      const topicQuizzes = sqlTopicQuizzes[topic.title];
      if (!topicQuizzes) {
  ...
  console.log(`\n--- SQL RE-SEED COMPLETE ---`);
  console.log(`Modules rebuilt:   ${sqlSections.length}`);
  ...
  console.log(`Final exam:        replaced ${oldExam.count} template questions → ${sqlFinalExam.length} distinct`);
}
