// Issue #102: internship template. Mirrors Certificate.tsx (training) exactly — keep both visually in sync.
// Consumes GET /api/certificate/internship/:internshipId (owner-or-admin) and renders the
// Internship Completion Certificate with the same frame/colors/fonts/QR/signature/print CSS
// as the training certificate. Only the certificate content differs.
...
const InternshipCertificate = () => {
  ...
  // Extract internshipId from URL query parameters: e.g. /internship-certificate?internshipId=<id>
  const getQueryParam = (name: string) => {
    const match = RegExp('[?&]' + name + '=([^&]*)').exec(window.location.search);
    return match ? decodeURIComponent(match[1]) : null;
  };
  const internshipId = getQueryParam('internshipId');

  useEffect(() => {
    if (!internshipId) {
      setError('No internship selected. Open this page from your internship record.');
      setLoading(false);
      return;
    }
    ...
    const fetchData = async () => {
      try {
        const res = await api.get(`/certificate/internship/${internshipId}`);
        setData(res.data);
      } catch (err: any) {
        console.error(err);
        setError(err.response?.data?.message || 'Failed to fetch certificate. Make sure the internship is completed and a certificate has been issued.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="py-20 text-center space-y-4 bg-slate-950 min-h-screen text-slate-400">
        ...
        <p className="text-sm font-semibold uppercase tracking-widest">Verifying internship credentials and rendering certified credentials...</p>
      </div>
    );
  }

  // Missing internshipId is a routing error for everyone (including ADMIN preview) —
  // there is no "default internship" the way /certificate defaults to courseId=C.
  if ((error || !data) && (user?.role !== 'ADMIN' || !internshipId)) {
    return (
  ...
  // Fallback for admin preview when the target internship has no certificate data yet.
  // PRIVACY (#100): never inject a fabricated person's identity — use the viewer's OWN
  // fields or neutral placeholders instead.
  ...
  const detailRows: Array<[string, string]> = [
    ['Internship Domain', displayData.domain || '—'],
    ['Role', displayData.role || '—'],
    ['Duration', displayData.duration || '—'],
    ['Internship Period', `${displayData.startDate} — ${displayData.endDate}`],
    ['Certificate ID', displayData.credentialId],
    ['Issue Date', displayData.endDate]
  ];
  ...
  return (
    <div className="py-6 flex flex-col items-center max-w-5xl mx-auto px-4 space-y-6 print-container animate-fade-in">
      ...
      <style>{`
        ...
        /* Internship detail rows (issue §2 Details Section) — same gold/navy design language */
        ...
        /* Badge Logo - White to match website */
        .cert-badge-logo {
            position: absolute;
            top: 35px;
            right: 45px;
        ...
        <div className="certificate-container">
            ...
            <div className="content">
                <div>
                    ...
                    <div className="cert-title">INTERNSHIP COMPLETION CERTIFICATE</div>
                    <div className="conferred-text">This is proudly conferred upon</div>
                </div>

                <div className="student-name">{displayData.name}</div>

                <div className="details-text">
                    This is to certify that <span className="highlight">{displayData.name}</span>,<br />
                    son/daughter of <span className="highlight">{displayData.fatherName || "N/A"}</span> of <span className="highlight">{displayData.collegeName || "N/A"}</span>, <span className="highlight">{displayData.branchName || "N/A"}</span>,<br />
                    has successfully completed the <span className="highlight">{displayData.domain || "N/A"}</span> Internship with EduNexus Pro during<br />
                    <span className="highlight">{displayData.startDate}</span> to <span className="highlight">{displayData.endDate}</span>.
                </div>

                <div>
                    <div className="course-box">{displayData.programTitle || `${displayData.domain || 'Professional'} Internship`}</div>
                </div>

                <div className="internship-details-box">
                    {detailRows.map(([label, value]) => (
                      <span className="id-row" key={label}>
                ...
                <div className="duration-performance">
                    {displayData.performanceGrade ? (
                      <>PERFORMANCE REVIEW: <span className="highlight" style={{ color: '#d4af37' }}>{displayData.performanceGrade}</span></>
                    ) : (
                      <>successfully completed the prescribed internship program</>
                    )}
                </div>
